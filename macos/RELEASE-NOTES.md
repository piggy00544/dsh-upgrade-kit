...
- 插件同步后自动重启 web 服务（让新插件代码立即生效，不再需要手动重启；App 探测掉线会自动重新拉起）
...
- 修复拖拽被官方输入框拦截：非图片文件拖入改为捕获阶段接管（不再出现"只支持图片"提示），拖入即上传并自动 @ 引用；纯图片拖拽保留官方原生体验

## v0.3.1 变更

- **底层升级：DeepSeek Harness 0.1.1-rc.1 → 0.1.1-rc.2**（官方视觉通道优化：Files API 图像上传复用 + 按模型要求自动缩放/转格式；五装备回归通过）

## v0.3.0 变更

- **全局拖拽上传**：文件拖到窗口任意位置即出现"松开上传"提示，松手直接传到当前会话（不再要求先打开附件面板）
